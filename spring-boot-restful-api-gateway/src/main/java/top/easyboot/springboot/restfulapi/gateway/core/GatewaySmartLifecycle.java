...
@Component
public class GatewaySmartLifecycle implements IGatewaySmartLifecycle, SmartLifecycle {

    /**
     * 每秒任务定时器
     */
    private Timer taskSecondTimer;
    /**
     * 是否已经运行
     */
     ...
     * PS：如果你想修改这个默认超时时间，可以按下面思路做，
     * 当然下面代码是springmvc配置文件形式的参考，
     * 在SpringBoot中自然不是配置xml来完成，这里只是提供一种思路。
     *
    ...
    @Override
    public void stop() {
        taskSecondTimer.cancel();
        System.out.println("stop");

        isRunning = false;
    }

    private void taskSecondRun(){
        System.out.println("taskSecondRun");
    }

    private void taskSecondStart(){
        try {
            if (taskSecondTimer!=null){
                taskSecondTimer.cancel();
            }
        }catch (Throwable e){
        }
        taskSecondTimer = new Timer();
        taskSecondTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                try {
                    taskSecondRun();
